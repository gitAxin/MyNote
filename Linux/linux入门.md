...
init─┬─abrtd
     ...
     ├─rpcbind
     ├─rsyslogd───3*[{rsyslogd}]
     ├─sshd───sshd───bash───bash───pstree  #在此查看
     └─udevd───2*[udevd]
[root@localhost ~]# exit
~~~
...
2. 双分支if条件语句
   ...
   #如果在定时任务中执行date=$(date +\%y\%m\%d),%号必须加转义符，因为在定时任务crontab文件中%号是有特殊含义的
...
- 查询已安装的服务
  ...
  - RPM包安装的服务
    ...
    #但是这条命令还附带的功能就是还可以查看所有RPM包安装的服务
    ~~~

    使用chkconfig --list 其中各数字代表的含义如下：
...
- 查看在系统中已经运行的进程
  ...
  #注意：像某些服务是手工进程，没有端口号的，使用netstat -tlun 查不出来。
  ~~~
...
3. 独立服务的自启动
   ...
   > 备注:/etc/rc.d/rc.local文件在系统启动之前(输入密码之前)都会执行这个文件，所以说，如果把服务启动的标准命令如：
   >
...
- 使用绝对路径，调用启动脚本来启动。不同的源码包的启动脚本不同。可以查看源码包的安装说明，查看启动脚本的方法。
  ...
  ~~~
  在源码包，INSTALL 文件中，有说明启动脚本的方法。
  同一个系统中可以安装两个apache ，一个是源码包安装，一个是RPM包安装，因为安装的路径不同，但是如果都启动，会报错，因为端口是一样的。
  ~~~
...
- 让源码包的apache服务能被service命令管理启动
  ...
  #把启动脚本链接到指定目录当中，就可以被service 识别 (因为service命令会搜索/etc/init.d/目录)
  
  service apache start
  #链接完成后，可启动/停止，但是并不推荐，因为容易搞混源码包安装的apache和RPM包安装的apache
  ~~~
...
- 让源码包的apache服务能被chkconfig与ntsysv命令管理自启动
  ...
  #添加完上面的注释后，执行此命令将apache添加到chkconfig管理序列
  
  chkconfig --list | grep apache
  #然后使用此命令就可以找到apache
  
  chkconfig apache on
  #当然，也就可以使用chkconfig apache on命令进行管理自启动了
  
  chkconfig --list | grep apache
  #再使用此命令就可以看到apache已经启动了。
  
  > 注意：虽然可以这么做，但是并不推荐，还是建议使用标准的启动方式（绝对路径）。因为容易搞混源码包安装的apache和RPM包安装的apache。
  ~~~
...
![](/../linux入门.assets/2018-06-09_086.png)
...
3. 查看系统中所有进程 ps/pstree

   ~~~
   [root@localhost ~]# ps aux
   #查看系统中所有进程，使用BSD操作系统格式
   #aux是Unix格式，不需要加“-”，如果加“-”,也可以正常执行，但是会显示一条警告信息。
   选项：	
   	a: 显示所有前台进程
   	x: 显示所有后台进程
   	u: 显示这个进程是由哪个用户产生的
...
5. 查看进程树
   ...
   init─┬─abrtd
        ...
        │      │             └─hald-addon-rfki
        │      └─{hald}
        ├─httpd───8*[httpd]  #这里代表，httpd有8个子进程
        ├─master─┬─pickup
        │        └─qmgr
        ├─mcelog
        ├─6*[mingetty]
...
3. pkill命令（同killall相似，一般 按照终端号踢出用户时使用此命令）
...
1. 把进程放入后台
   ...
   - 方法一：
     ...
     #此方法放到后台，命令在后台是执行的
     #像top、vim命令就算是在后面加了&符在后台也是暂停的，因为命令是和前台有交互的。到后台执行是无意义的。
     ~~~
   ...
   - 方法二：
     ...
     ~~~
     [root@localhost ~]# top
     #在top（top:查看系统健康状态的命令）命令执行的过程中，按下ctrl+z快捷键
     #此方法放到后台，命令在后台是暂停的
     ~~~
   ...
   选项：
   	-l: 显示工作的PID
   注：	“+”号代表最近一个放入后台的工作，也是工作恢复时，默认恢复的工作。“-”号代表倒数第二个放入后台的工作，当然也可以指定工作号来恢复工作。
...
1. vmstat命令监控系统资源 （此命令其实是和top命令结合了一下）
   ...
   | 进程数 | 内存的使用（重要） | 交换分区的 | 硬盘输入输出 | 系统资源的使用 | CPU的使用（重要） |
   | ------ | ------------------ | ---------- | ------------ | -------------- | ----------------- |
   |        |                    |            |              |                |                   |
...
7. 判断当前系统的位数
   ...
   #file命令是查看文件类型， 使用file命令查看一个系统外部命令，如ls，就间接的可以看到Linux操作系统的位数。
   ~~~
...
1. crond服务管理与访问控制
   ...
   #启动命令，一般都是默认安装并启动的，不需手动启动
   
   [root@localhost ~]# chkconfig crond on
   #自启动命令，一般也都是默认安装并自启动的，不需手动启动
   ~~~
...
#查看服务是否启动，一般系统都是默认安装并启动的。

[root@localhost ~]# chkconfig --list | grep rsyslog
#查看服务是否自启动，一般系统都是默认安装并自启动的。注意：rsyslog后面没有“d”
~~~
